...
Classical baseline parameters were tuned via grid search over
$\{5,10,15,20,25,30\}$ for base-stock and a smaller grid for $(s,S)$;
finer grids or Bayesian optimisation may yield stronger classical baselines.
We define the bullwhip ratio as $\text{Var}(\text{orders}_i)/\text{Var}(d_t)$
using end-customer demand $d_t$ in the denominator for all nodes; the
standard echelon-level definition uses each node's immediate downstream
demand instead, which would give lower absolute values for upstream nodes
but preserve all relative comparisons reported here.
